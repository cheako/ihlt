...
/**
 * @file server.c
 * @brief using select() for I/O multiplexing
 * @author Mike Mestnik
 *
 * Example from:
		...
		if (str1 == NULL ) {
			if (ntoken != NULL || endswell) {
				...
				for (str2 = token;; str2 = NULL ) {
					...
					argv[argc++] = subtoken;
					if (t != NULL ) {
						size_t n = strlen(t) + strlen(subtoken) + 4;
						i = NULL;
						while (i == NULL )
							i = realloc(t, n);
						t = i;
...
void OpenConnection(int listener, int *fdmax, fd_set *master, char *name) { /* we got a new one... */
...
int main(int argc, char *argv[]) {
	...
	for (;;) {
		...
		if (FD_ISSET(listener, &read_fds))
			OpenConnection(listener, &fdmax, &master, argv[0]);
